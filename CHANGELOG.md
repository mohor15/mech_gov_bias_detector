...
milestone since M0 has pointed at and never built: **application-level
encryption at rest** for a fixed, named list of columns proven never to
participate in any SQL comparison (`evidence_chain.payload`, both review
tables' `resolution_notes`, `protected_attribute_resolutions.proxy_basis`);
**retention tiers** for the only two tables that are simultaneously
non-evidentiary and privilege-unlockable (`shadow_findings`,
`protected_attribute_resolutions`); and **privilege classification**
closing the gap `docs/milestones/M9.md` §9.5 disclosed and left open —
extending `evidence_chain`'s own `REVOKE UPDATE, DELETE` lockdown to
...
migrations, almost no new persisted state. **Zero new external
dependencies** — the `cryptography` package added at M5 for signing is
sufficient. Full design rationale, this milestone's own three-pass
hostile-review record (two blocking findings reversed before
implementation began, seven amendments folded in, four further findings
from a second design-review pass, and the implementation-time findings
below), and the post-implementation production-readiness review:
[`docs/milestones/M11.md`](docs/milestones/M11.md).

### Added
- `audit/encryption.py` — `FernetFieldEncryptor`/`NoOpFieldEncryptor` (both satisfying the `FieldEncryptor` protocol), `FieldDecryptionError`, `is_encrypted`/`decrypt_field` (the one place every read path checks the `"gpenc1:"` version-prefix marker before consulting any encryptor), and `load_encryptor(key)`. Deliberately no ephemeral, auto-generated fallback when unset, unlike `audit.signing.load_signer` — a fresh, per-process encryption key would make already-encrypted data unrecoverable on the next restart, a failure mode signing's identical-looking fallback does not share.
- `Settings.FIELD_ENCRYPTION_KEY: str | None` — a Fernet key (`Fernet.generate_key()`'s own base64 output, not `SIGNING_PRIVATE_KEY`'s hex convention). Unset (default) means encryption is off; a `WARNING`-level (not `INFO`) log line names this on every affected startup.
- `Settings.RETENTION_DAYS_SHADOW_FINDINGS`/`RETENTION_DAYS_PROTECTED_ATTRIBUTE_RESOLUTIONS: int | None` — no numeric default for either; unlike this platform's other governance-sensitive constants (the EEOC ratio, the CFPB threshold, the *Castaneda* convention), no external standard exists to source a defensible retention period from. Unset means that table's retention is disabled.
- `retention/purge_expired_records.py` — the retention CLI. Mirrors `population_engine/run_policies.py`'s/`human_review/backfill_reviews.py`'s explicitly-invoked, no-daemon shape. Deletes rows older than each table's configured cutoff, one transaction per table; a table with no configured window is skipped entirely, logged as such. Idempotent: a re-run against an already-purged table deletes zero rows. Runs under a separate, elevated database connection (`--database-url`, never `Settings.DATABASE_URL`) — the app's own `gov_platform_app` role has no `DELETE` privilege on either retention-eligible table as of migration `0026`.
- Migrations `0025`/`0026` — `0025` extends the `evidence_chain`-style privilege lockdown to `systems`, `model_versions`, `decision_events`, `findings`, `verdicts`, `verdict_findings`; `0026` revokes `DELETE`/`UPDATE` from `gov_platform_app` on `shadow_findings`/`protected_attribute_resolutions` and adds the two indexes (`evaluated_at`, `resolved_at`) the retention CLI's own cutoff scans need. Regrouped by risk profile, not by table count: `0025`'s six tables include this platform's actual highest-throughput, request-path-critical ones and need a maintenance window (`GRANT`/`REVOKE` takes an `ACCESS EXCLUSIVE` lock in Postgres); `0026`'s two low-write-volume tables don't.
- `tests/unit/audit/test_encryption.py`, `tests/unit/retention/test_purge_expired_records.py` — pure-logic coverage (Fernet round trips, marker semantics, no-op behavior, `compute_cutoff`, CLI argument wiring), no database needed.
- `tests/integration/test_m11_encryption_postgres.py`, `tests/integration/test_purge_expired_records_postgres.py` — real Postgres round trips for all four encrypted columns (including decrypt-failure containment for each), `verify_chain --encryption-key`, and retention purge/skip/idempotency behavior. New `admin_database_url`/`admin_db_engine` test fixtures and a `requires_admin_postgres` marker (`tests/conftest.py`) provide the elevated connection these need, gated by a new `ADMIN_DATABASE_URL` environment variable (CI already sets it — the same connection migrations use).
- `tests/integration/test_privilege_enforcement.py` — extended with `UPDATE`/`DELETE`-rejection tests for all six of migration `0025`'s tables and both of `0026`'s, plus a positive control proving the elevated connection can still delete where the restricted role cannot.

### Changed
- `audit/evidence_store.py` — `EvidenceStore` gains an optional `encryptor` collaborator. `append` now encrypts the canonical payload string *after* `record_hash` is computed over it (hash-then-encrypt, not encrypt-then-hash — the hash chain's own guarantee is unaffected). `_to_model` (now an instance method, was `@staticmethod`) decrypts first, falling through to plaintext for a pre-M11/unmarked row; a decrypt failure raises `FieldDecryptionError`, named with the specific `sequence_number` that failed, and is never papered over with a placeholder — `payload` is evidentiary content. The same `encryptor` is threaded into `ProtectedAttributeResolutionRepository` and the default-constructed `VerdictReviewRepository`.
- `db/repositories/verdict_review.py`, `db/repositories/population_finding_review.py` — both gain an optional `encryptor` collaborator; `resolve()` encrypts `resolution_notes` before writing it; `_to_model` decrypts it on read with per-record containment (logged, substituted with `"[unreadable]"` for that one row) — the identical "one record's failure never aborts the rest" discipline `human_review/backfill_reviews.py` already holds itself to. `reviewer` is untouched — plaintext, exactly as before, since `resolve()`'s own equality check against it cannot survive a non-deterministic cipher.
- `db/repositories/protected_attribute_resolution.py` — gains an optional `encryptor` collaborator encrypting/decrypting `proxy_basis` the same way, with the same per-record containment. `attribute_name` is untouched — `list_by_decision_event`'s `ORDER BY` depends on its plaintext order.
- `audit/verify_chain.py` — `verify_chain_from_database` gains `encryption_key`; the CLI gains `--encryption-key`. A `FieldDecryptionError` from `EvidenceStore.all()` (no key, wrong key, or corrupted ciphertext) is caught and reported as a clean, distinct `ChainVerificationResult` (`valid=False`, naming the specific unreadable record), never an unhandled crash. The hash-recomputation logic itself is unchanged — it already operated on the decrypted `EvidenceRecord.payload` dict, never on raw column bytes.
- `api/app.py` — builds one `FieldEncryptor` from `Settings.FIELD_ENCRYPTION_KEY` and threads it into every repository that touches an encrypted column; logs a `WARNING` on every startup where the key is unset.

### Fixed
_(found and corrected during this milestone's own three-pass hostile review, before implementation began — see `docs/milestones/M11.md`'s "Revision note" for the full record)_
- The first design draft proposed encrypting `reviewer` (both review tables) and `protected_attribute_resolutions.attribute_name`, checked only against the API router layer's own `WHERE`/`GROUP BY` clauses. The repository layer underneath was never checked: `resolve()`'s own conditional `UPDATE ... WHERE ... AND reviewer = :reviewer` (M9's identity-continuity check) cannot survive comparison against Fernet's non-deterministic ciphertext — every legitimate `resolve()` call would have failed with `rowcount == 0`, permanently, the moment encryption was enabled, deleting the Human Review Workflow's own terminal action rather than merely degrading it. `attribute_name` would have broken `list_by_decision_event`'s `ORDER BY` the same way, at lower severity. Fixed before implementation: both removed from the encrypted-field set; the corrected list is exactly four columns.
- The first design draft claimed `audit/verify_chain.py` needed "zero code changes." False as stated: `verify_chain_from_database` constructs `EvidenceStore(engine)` with no other argument, and neither that constructor nor the CLI's own `argparse` `main()` had any mechanism to obtain an encryption key from anywhere — left as specified, the tool would have crashed with an unhandled `json.JSONDecodeError` on the first ciphertext row it reached. Fixed before implementation: the `--encryption-key`/`encryptor` plumbing and the specific, catchable `FieldDecryptionError` handling described above.
- A second design-review pass, after the corrected draft was otherwise approval-ready, found the application-role privilege claim in §1/§5.3 was self-contradicting — it asserted both "unchanged by this milestone" and "never deletable" about the same six ordinary admin-configuration tables (`plugin_registrations`, `policy_bindings`, `protected_attribute_rules`, `population_policy_bindings`, `verdict_reviews`, `population_finding_reviews`), which retain their full, unrevoked `DELETE` grant. Corrected before implementation: the privilege claim is scoped to exactly the ten tables this milestone actually classifies, not "this schema" as a whole.
- The same pass found retention's "nothing irreplaceable is lost" premise for `protected_attribute_resolutions` had never been re-checked against M5's own architecture: the live resolver consults a DB-backed, `UPDATE`/`DELETE`-grantable `protected_attribute_rules` table with no `population_findings.classification_snapshot`-equivalent, so "re-running the resolver reproduces it" is conditional on that ruleset never changing, not an unconditional guarantee. Retention scope is unchanged (the raw attribute values persist forever regardless, in `decision_events`/`evidence_chain.payload`); the caveat is now disclosed rather than silently assumed away.
- **Implementation-time**: `EvidenceStore.all()` decrypts every row before returning any of them, so a decrypt failure can't be caught inside `verify_chain()`'s own per-record hash-checking loop the way the design doc's prose literally suggested — the loop never starts. Resolved by catching `FieldDecryptionError` around the whole `store.all()` fetch in `verify_chain_from_database` instead, with the specific failing record's `sequence_number` named in the exception message itself (set by `EvidenceStore._to_model`) so the result stays actionable even though `checked_count` can't reflect hash-verification progress that never ran. Not a design contradiction — a natural consequence of `EvidenceStore.all()`'s pre-existing eager, list-returning contract that the design didn't spell out at this level of detail.
- **Implementation-time**: writing Postgres integration tests for encrypted `evidence_chain` rows surfaced a real test-isolation hazard, not present before M11: `EvidenceStore.all()`/`verify_chain_from_database` scan the *entire*, shared, cumulative `evidence_chain` table with no per-test scoping (a pre-existing characteristic). A test-only encrypted row left behind under a one-off key would have silently broken every *other* test's own keyless or differently-keyed verification call, including the pre-existing `test_verify_chain_postgres.py` suite. Fixed: every such test now cleans up its own row via the elevated `admin_db_engine` connection before returning (`evidence_chain` has no `DELETE` grant for the ordinary role — unrelated to M11, unchanged since migration `0008`).
- **Implementation-time**: `FernetFieldEncryptor.decrypt_token` called `token.encode("ascii")` with no containment around it. Found and confirmed by direct reproduction during this milestone's own post-implementation hostile review: a `resolution_notes`/`proxy_basis` value that happens to collide with the `"gpenc1:"` marker (§5.1's own already-disclosed risk — free text submitted before encryption was ever enabled) *and* contains non-ASCII characters raised an uncaught `UnicodeEncodeError` instead of the intended `FieldDecryptionError`, bypassing every caller's per-record containment (a crashed list response, not a `"[unreadable]"` placeholder). Fixed: `UnicodeEncodeError` is now caught at the same point and wrapped into `FieldDecryptionError`, identically to `InvalidToken`. Regression test added (`test_a_non_ascii_marker_collision_value_raises_field_decryption_error_not_a_crash`).
- **CI-found**: migration `0026`'s comment *"Supports `"delete rows older than :cutoff"` for the new retention CLI"* failed with `sqlalchemy.exc.InvalidRequestError: A value is required for bind parameter 'cutoff'` — `db.migrate.apply_migrations` applies each file via SQLAlchemy's `text()`, which scans the *entire* string for `:name`-shaped placeholders regardless of SQL comment syntax, the identical class of defect first found and fixed at M7 (migrations `0017`–`0019`, which now each carry an explicit warning comment against exactly this). This one was never caught locally — this sandbox has no working Postgres/Docker, so `apply_migrations` was never actually exercised against either new migration before CI ran it. Fixed by rewording the comment ("a given cutoff timestamp") and adding the same warning migration `0017` already carries.
- **CI-found**: every M11 test depending on the new `admin_db_engine`/`admin_database_url` fixtures (15 tests) plus `retention/purge_expired_records.py`'s own CLI (1 further test) failed with `ModuleNotFoundError: No module named 'psycopg2'`. Root cause: `db.session.create_db_engine` passed its URL straight to `sqlalchemy.create_engine` with no driver normalization; `ADMIN_DATABASE_URL` is a deliberately bare `postgresql://`-scheme URL in CI (shared with `infra/ci/setup_test_role.py`, which hands it directly to `psycopg.connect()` and needs the plain scheme), and SQLAlchemy's own default dialect for a bare `postgresql://` scheme is `psycopg2` — not a dependency of this project (only `psycopg`, v3, is) and not installed. `db/migrate.py` already solved this identical problem for its own `--database-url` argument at M1 (`_with_psycopg_driver`), but that fix lived only there — nothing protected `create_db_engine`'s other callers, and `POSTGRES_URL` never exercised the gap because it was already driver-qualified in `ci.yml`. Fixed by moving the normalization into `db.session.with_psycopg_driver`, applying it inside `create_db_engine` itself (protecting every caller, not just one CLI), and having `db/migrate.py` import the shared function instead of keeping its own copy — `db/migrate.py`'s own behavior (no `connect_timeout`, a deliberate, separate design decision) is otherwise unchanged. Regression coverage added in a new `tests/unit/db/test_session.py`.

### Deferred
Chain pruning/checkpointing for `evidence_chain`/`population_findings` — the single largest disclosed gap this milestone leaves open, unassigned to any future milestone until a concrete legal retention requirement or storage-cost pressure forces the question. A rule-versioning/`classification_snapshot`-equivalent mechanism for `protected_attribute_rules`/`protected_attribute_resolutions` (the conditional-reconstructibility caveat above). Key rotation and KMS/HSM-backed custody for either the signing key or the new encryption key. Retroactive encryption of any pre-M11 row. A deterministic/blind-indexed encryption scheme for `reviewer` specifically. A general "which columns are sensitive" classification registry (a fixed, named list in code is sufficient at this schema's current size). A new Admin API surface for retention/encryption status. Real, provider-specific storage-encryption infrastructure-as-code. Authentication anywhere (still M13, now a ninth-consecutive-milestone gap). Multi-tenancy/multi-plane deployment/mTLS (M13).
