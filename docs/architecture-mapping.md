# Architecture → Module Mapping (M13)
...
| §15 APIs | `api/health.py`, `api/readiness.py`, `api/auth.py`, `api/ingestion/routes.py`, `api/admin/systems.py`, `api/admin/plugins.py`, `api/admin/policy_bindings.py`, `api/admin/protected_attribute_rules.py`, `api/admin/population_policy_bindings.py`, `api/admin/population_findings.py`, `api/admin/metrics.py`, `api/admin/verdict_reviews.py`, `api/admin/population_finding_reviews.py`, `api/app.py`, `api/asgi.py` | Ingestion routes are registry-generated (one per registered adapter — currently `synthetic` and `credit-scorecard`), **unchanged by M7/M8/M9/M11**, same response shape; `/healthz` also unchanged. `GET /readyz` and `GET /v1/admin/metrics` (M7) unchanged by M8/M9/M11 — every JSON response shape any client sees is byte-for-byte identical to before M11; encryption/decryption happens entirely at the repository read/write boundary. Ten endpoints across two new routers (`verdict-reviews`, `population-finding-reviews`, M9; `list`/`get`/`claim`/`release`/`resolve` each) — read-only except for the three workflow-transition actions, which never load new code or compute a new result, the same boundary every Admin API in this codebase draws. `claim`/`release`/`resolve` translate a repository-raised `ValueError` conflict into a `409` via an explicit handler in each route, not the codebase's existing global `ValueError → 422` handler. `app.py` is still the side-effect-free factory; **M11** builds one `FieldEncryptor` from `Settings.FIELD_ENCRYPTION_KEY` and threads it into every repository that needs it. **New in M13 — authentication is real.** Two new endpoints (`POST /v1/auth/session`/`logout`) issue/clear an `HttpOnly` session cookie for the M10 dashboard. Every existing endpoint's *request-side* contract changes (a valid `Authorization: Bearer <token>` header or session cookie is now required, gated per-route or per-router by `require_role`), while every existing *response body* shape stays byte-for-byte unchanged — the first milestone where "zero API changes" cannot be claimed unconditionally. `/healthz`/`/readyz` remain the two deliberate exceptions (orchestrator probes). `claim`/`resolve`'s `reviewer` request field becomes optional (not removed); once authenticated, the persisted value is always the authenticated principal's own name, a client-supplied disagreeing value rejected with `422` — see M13-specific notes. |
...
| §17 Deployment Architecture | `infra/docker/` | Single container, single service; assumes an external Postgres — unchanged by M7/M8/M9/M11/M13. A real orchestrator can use `GET /readyz` as an actual readiness probe rather than reusing `/healthz` (liveness only) for both purposes. **M11**: storage-level encryption of that external Postgres volume — the literal, standard reading of "encryption at rest" for the primary datastore — is a documented deployment requirement this repository cannot itself provision or verify (no Postgres provisioning exists in this repository's own infrastructure); `retention/purge_expired_records.py`'s own recurring-schedule invocation (a cron/`CronJob` an operator sets up) is real scheduling scope, also unbuilt here, for the identical reason `population_engine/run_policies.py`'s batch cadence already is. **M12**: `reporting/generate_report.py`'s own recurring-schedule invocation is the identical, third instance of this same unbuilt scope — no daemon, no in-process scheduler, an operator's own cron/`CronJob` cadence, unchanged. **M13**: closes the *authentication* half of this platform's standing gaps only — multi-plane topology, multi-tenancy, and mTLS remain explicitly this section's own scope, deliberately not absorbed into M13 despite sharing a citation (see M13-specific notes); transport-layer (TLS) security remains the identical, unaddressed deployment-layer assumption it always was. |

## M13-specific notes

- **"M13" was never one deliverable — this milestone's own central
  finding, argued in full in `docs/milestones/M13.md` §4.2.** Grepped
  directly across all twelve prior milestone documents: the word "M13"
  was used for at least three structurally different concerns
  (caller-identity/permission-checking; architecture §17's own
  deployment-topology scope — multi-tenancy, mTLS, multi-plane;
  operational scheduling infrastructure for this platform's three batch
  CLIs) that happened to share a number only because eleven consecutive
  design documents each deferred whatever cross-cutting concern they
  individually ran into. This milestone scopes itself to exactly one of
  the three — authentication and authorization — and leaves the other
  two explicitly unassigned, not silently absorbed.
- **`AUTH_ENABLED` defaults to `True` (fail-closed) — a deliberate,
  disclosed break from every prior opt-in `Settings` field's default-
  `False` precedent, approved 2026-07-31.** `FIELD_ENCRYPTION_KEY`/
  `RETENTION_DAYS_*` defaulting to "off" only weakens confidentiality of
  data already written, or retention hygiene — real but bounded harms.
  An unauthenticated caller could otherwise fabricate ingestion events,
  silently deactivate a policy binding governing real financial
  decisions, or falsely resolve a bias-finding review under someone
  else's name — the single sharpest standing risk eleven consecutive
  design documents named. See `docs/milestones/M13.md` §13.1.
- **Named `access/`, not `auth/` — a real naming-collision risk found
  and corrected before implementation, not a stylistic preference.**
  `auth/` would have sat immediately adjacent, alphabetically and
  visually, to the existing `audit/` package (`hash_chain.py`,
  `evidence_store.py`, `verify_chain.py`, `signing.py`, `encryption.py`)
  — the same conceptual neighborhood (secret-material handling) this
  milestone's own `access/tokens.py` also occupies, a realistic source of
  a mis-typed import. The **API path prefix** (`/v1/auth/session`,
  `/v1/auth/logout`) is a separate namespace with no equivalent
  collision risk and is unaffected by the package rename.
- **Opaque, DB-backed, revocable tokens — not JWTs, not OAuth2, no
  external identity provider.** A compromised or offboarded principal's
  access ends the moment `PrincipalRepository.revoke` commits, with no
  stateless-token blocklist to maintain — the direct, checked benefit
  over a JWT for this platform's real, disclosed operating scale (a
  handful of admin/reviewer principals). This platform has never
  integrated an external service for anything (M6 declined a dedicated
  analytical store, M7 declined Prometheus/OpenTelemetry, M12 declined a
  document-rendering service); the identical "no infrastructure without
  a concrete forcing case" reasoning applies here.
- **Closes `docs/milestones/M9.md`'s own named, sharpest gap — an
  unauthenticated caller fabricating a specific person's professional
  judgment about a real bias finding — with no schema change.** Once a
  caller is authenticated, `claim`/`resolve`'s persisted `reviewer` is
  always the authenticated principal's own name, never the request body;
  a client-supplied value that disagrees with it is rejected with `422`,
  never silently discarded. **Prospectively only** — historical
  `reviewer` values already persisted before a given deployment first
  enables `AUTH_ENABLED=true` remain exactly as recorded, permanently
  unverifiable, per this platform's own "never silently rewrite an
  already-recorded fact" discipline (M5 §13.6). A disclosed, permanent
  residual limitation, not a defect any future milestone can close.
- **This platform's own CLIs (`db/migrate.py`, `plugins/seed_registry.py`,
  `population_engine/run_policies.py`, `retention/purge_expired_records.py`,
  `reporting/generate_report.py`, `human_review/backfill_reviews.py`,
  `audit/verify_chain.py`, `audit/verify_population_findings.py`) are
  deliberately not authenticated by this milestone** — they keep their
  existing, separate, database-credential-based trust boundary, the
  identical one this project has treated as sufficient for CLI-invoked
  operations since M1. `access/create_principal.py` (the CLI that
  provisions/revokes a `Principal`) inherits the identical boundary,
  consistently.
- **A second, independent hostile-review pass — performed directly
  against the running codebase, not just this design document's own
  prose — found four blocking defects before implementation began**: a
  missing `GRANT` statement in the frozen migration SQL (would have made
  `access/create_principal.py`'s first `INSERT` fail against a
  freshly-migrated database); the recommended `AUTH_ENABLED=True`
  default would have broken this repository's own `docker-smoke` CI job
  and its entire pre-existing ~640-test Postgres-gated suite, both now
  addressed as explicit, in-scope deliverables (§8/§5.7 of
  `docs/milestones/M13.md`); and an underspecified `reviewer`-field
  mechanism that would have let a client-supplied value silently diverge
  from what actually got persisted. See `docs/milestones/M13.md`'s own
  Revision note, items 6–15, for the full record.
